...
**这是一个安全关键 AI 方向的完整工程项目。** 它展示的不是"会调 API",而是:
...
|------|----------|-----------|--------|----|----------|
| 纯规则引擎(无图谱) | 93.7% | 97.1% | 91.7% | 94.3% | 149/191 |
| **图谱+规则** | 94.2% | 97.1% | 92.7% | **94.8%** | **166/191** |

> **别只看 F1**:图谱相对纯规则引擎,二分类 F1 只涨了 **+0.5pp**(94.3% → 94.8%),
> 但精确匹配从 149/191 涨到 166/191,**+8.9pp**。
> 图谱真正的贡献不在"有没有风险"这个二分类上(规则引擎基本已经能判),
> 而在**分得清是 critical 还是 high** 这个多分类粒度上 —— 报数字时要说清楚是哪一个。

> 主集从 77 扩到 191 后 F1 更诚实:召回 92.7% 而非虚高 100%。生成样本仍用于回归,不单独证明泛化。

> Embedding RAG 的 80.3% 是 **77 样本旧评测集**上的结果(见 [RAG_COMPARISON_REPORT.md](./RAG_COMPARISON_REPORT.md)),
> 与这张 191 样本的表**不是同一批数据,不能直接相减**。不宜笼统说"图谱比 RAG 高 14 个点"。
...
> ⚠️ **n=20 的量级**:95.0% 就是 20 条里对 19 条。这个样本的 95% 置信区间大约是
> **75% ~ 99%** —— 方向可信(语义评估确实有用),但**别把 25pp 的提升当精确结论讲**。
> 更稳妥的说法:"在 20 条规则覆盖不到的难例上,完整 Agent 路径精确匹配 16/20,
> 且没有把原本判对的改错(弄坏 0 条)";后一句比前一句更有说服力。
...
剩余误差全部是 severity 边界（high vs critical 的过度升级/降级），不是「有没有风险」的漏检。这比主集虚高 F1 更有说明价值。
...
## 🔗 与 deep_research_agent 的定位区分

> deep_research 那个项目展示的是**多智能体编排**——用 LangGraph Send 并行调度多个研究员。这个项目展示的是**安全关键AI + LangChain 深度使用**——用 structured output 做处方解析,tool calling 做图谱交互,规则引擎兜底保障安全。
>
...
| high 等级识别偏弱 | 主集 high 类 F1 **65.0%**,但**支持数仅 16 条** | 样本太少,这个数字置信区间很宽(16 条里错 1 条就跳 6pp),**不宜把它当作可靠的类别指标** |
